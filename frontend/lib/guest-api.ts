...
// The full-gallery ZIP is now built in the browser (client-zip, streamed to disk
// via the File System Access API with a Blob fallback), so the old
// mark-zip-as-downloaded / request-zip-generation calls are gone. The backend
// endpoints remain (dormant) in case server-side zipping is re-enabled.
