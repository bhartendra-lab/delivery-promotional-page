 ...
 * liked, shortlists the best picks for delivery, and downloads. No cover banner
 * or event-detail header here (that lives on the Media tab); no delete either.
 */
export function SmartSelectsTab({ loading }: { loading: boolean }) {
  ...
  return (
    <section className="flex min-w-0 flex-1 flex-col px-6 pb-12 pt-6 sm:px-10">
      {/* Header — title + Locate Original Images action. */}
      ...
      {loading && media.length === 0 ? (
        ...
        <>
          <div className="mb-4 flex flex-wrap items-baseline gap-x-2 gap-y-1 text-[12.5px] text-[var(--color-brand-muted)]">
            <span className="inline-flex items-center gap-1.5">
              <IconHeart size={13} className="text-[var(--color-brand-navy)]" />
              <span className="font-semibold text-[var(--color-brand-ink)]">
                {likedCount.toLocaleString("en-IN")}
              </span>
              liked
            </span>
            <span aria-hidden>·</span>
            <span className="inline-flex items-center gap-1.5">
              <IconStar size={12} filled className="text-[var(--color-brand-warning)]" />
              <span className="font-semibold text-[var(--color-brand-ink)]">
            ...
            {filtersActive && (
              <>
                <span aria-hidden>·</span>
                <span>{totalForView.toLocaleString("en-IN")} shown</span>
              </>
            )}
          </div>
