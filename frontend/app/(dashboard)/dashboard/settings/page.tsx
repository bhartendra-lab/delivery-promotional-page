  ...
  return (
    <div className="space-y-8 dash-rise">
      <div>
        <a
          href="/dashboard"
          className="mb-4 inline-flex items-center gap-1.5 text-xs font-medium text-[var(--color-brand-muted)] hover:text-[var(--color-brand-navy)]"
        >
        ...
        <h1
          className="mt-1 text-3xl font-semibold text-[var(--color-brand-ink)]"
...
function CompanyForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Save bar */}
      <div className="flex items-center justify-between gap-4 rounded-xl border border-[var(--color-brand-border)] bg-white px-5 py-4">
        {saveState === "error" && errorMsg ? (
          ...
          <p className="flex items-center gap-2 text-sm text-[#38a169]">
            <CheckIcon className="h-4 w-4 shrink-0" />
            Changes saved successfully
          </p>
        ) : (
          <p className="text-sm text-[var(--color-brand-muted)]">
            All changes are saved immediately across delivery pages.
          </p>
        )}
        ...
        <button
          ...
          className="brand-focus group relative inline-flex h-11 shrink-0 items-center gap-2 overflow-hidden rounded-xl bg-[var(--color-brand-navy)] px-6 text-sm font-semibold text-white shadow-[0_8px_24px_-12px_rgba(15,45,92,0.4)] transition-transform hover:-translate-y-0.5 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60"
        >
          <span
            aria-hidden
...
function Card({
  ...
  return (
    <div className="rounded-2xl border border-[var(--color-brand-border)] bg-white p-6 shadow-[0_2px_12px_-4px_rgba(15,45,92,0.08)]">
      <div className="mb-5 flex items-center gap-2.5">
        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-brand-navy)]/8 text-[var(--color-brand-navy)]">
          {icon}
        </span>
  ...
  return (
    <label className={`block ${className}`}>
      ...
      <input
        ...
        required={required}
        className="h-11 w-full rounded-xl border border-[var(--color-brand-border)] bg-[var(--color-brand-bg)] px-3.5 text-sm text-[var(--color-brand-ink)] outline-none placeholder:text-[var(--color-brand-muted)]/60 focus:border-[var(--color-brand-navy)] focus:bg-white focus:shadow-[0_0_0_3px_rgba(15,45,92,0.1)]"
      />
    </label>
  );
}
...
function FetchError({ message }: { message: string }) {
  return (
    <div className="flex items-start gap-3 rounded-xl border border-[var(--color-brand-danger)]/30 bg-[var(--color-brand-danger-soft)] p-5 text-sm text-[var(--color-brand-danger)]">
